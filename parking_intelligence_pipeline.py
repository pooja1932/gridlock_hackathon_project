...
DEFAULT_INPUT = Path(
    os.environ.get(
        "PARKSENSE_INPUT",
        ROOT / "data" / "jan_to_may_police_violation_anonymized.csv",
    )
)
OUT = ROOT / "outputs"
...
def load_and_clean(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"Input CSV not found: {path}. Pass --input <csv_path> or set PARKSENSE_INPUT."
        )
    df = pd.read_csv(path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build ParkSense AI hotspot outputs.")
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="Path to the anonymized police violation CSV.",
    )
